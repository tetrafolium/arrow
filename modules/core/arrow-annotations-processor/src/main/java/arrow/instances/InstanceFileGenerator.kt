...
data class Instance(
    ...
    val expandedArgs: String = args.joinToString(
        separator = ", ",
        transform = {
            "${it.first}: ${it.second}"
        }
...
class InstanceFileGenerator(
    ...
    private fun genCompanionExtensions(i: Instance): String =
        """|
                |fun ${i.expandedTypeArgs(reified = false)} ${i.receiverTypeName}.Companion.${i.companionFactoryName}(${(
            i.args.map {
                "${it.first}: ${it.second}"
            } + (if (i.args.isNotEmpty()) listOf("@Suppress(\"UNUSED_PARAMETER\") dummy: kotlin.Unit = kotlin.Unit") else emptyList())
            ).joinToString(", ")
