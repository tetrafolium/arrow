...
object MonadWriterLaws {
    ...
    inline fun <reified F, reified W> laws(
        MF: Monad<F>,
        MW: MonadWriter<F, W>,
    ...
    inline fun <reified F, reified W> monadWriterWriterPure(
        MW: MonadWriter<F, W>,
        MOW: Monoid<W> = monoid<W>(),
        EQ: Eq<Kind<F, Int>>
    ) {
    ...
    inline fun <reified F, reified W> monadWriterTellFusion(
        genW: Gen<W>,
        MW: MonadWriter<F, W>,
        MOW: Monoid<W> = monoid<W>()
    ) {
    ...
    inline fun <reified F, reified W> monadWriterListenPure(
        MW: MonadWriter<F, W>,
        MOW: Monoid<W> = monoid<W>(),
        EqTupleWA: Eq<Kind<F, Tuple2<W, Int>>>
    ) {
    ...
    fun <F, W> monadWriterListenWriter(
        genTupleWA: Gen<Tuple2<W, Int>>,
        MW: MonadWriter<F, W>,
        EqTupleWA: Eq<Kind<F, Tuple2<W, Int>>>
    ) {
