...
object AsyncLaws {
    inline fun <reified F> laws(AC: Async<F> = async(), EQ: Eq<Kind<F, Int>>, EQ_EITHER: Eq<Kind<F, Either<Throwable, Int>>>, EQERR: Eq<Kind<F, Int>> = EQ): List<Law> =
    ...
    inline fun <reified F> asyncSuccess(AC: Async<F> = async(), EQ: Eq<Kind<F, Int>>): Unit =
        forAll(
            Gen.int(),
            { num: Int ->
    ...
    inline fun <reified F> asyncError(AC: Async<F> = async(), EQ: Eq<Kind<F, Int>>): Unit =
        forAll(
            genThrowable(),
            { e: Throwable ->
