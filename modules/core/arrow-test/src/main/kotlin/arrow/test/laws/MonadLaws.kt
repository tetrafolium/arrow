...
object MonadLaws {
    ...
    inline fun <reified F> leftIdentity(M: Monad<F> = monad<F>(), EQ: Eq<Kind<F, Int>>): Unit =
        forAll(
            genFunctionAToB<Int, Kind<F, Int>>(genApplicative(Gen.int(), M)), Gen.int(),
            { f: (Int) -> Kind<F, Int>, a: Int ->
    ...
    inline fun <reified F> mapFlatMapCoherence(M: Monad<F> = monad<F>(), EQ: Eq<Kind<F, Int>>): Unit =
        forAll(
            genFunctionAToB<Int, Int>(Gen.int()), genApplicative(Gen.int(), M),
