...
object AlternativeLaws {
    ...
    inline fun <reified F> alternativeLeftDistributivity(AF: Alternative<F>, crossinline cf: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>): Unit =
        forAll(
            genConstructor(Gen.int(), cf), genConstructor(Gen.int(), cf), genFunctionAToB<Int, Int>(Gen.int()),
    ...
    inline fun <reified F> alternativeRightDistributivity(
        AF: Alternative<F>,
        crossinline cf: (Int) -> Kind<F, Int>,
        crossinline cff: (Int) -> Kind<F, (Int) -> Int>,
        EQ: Eq<Kind<F, Int>>
        ...
        forAll(
            genConstructor(Gen.int(), cf), genConstructor2(Gen.int(), cff), genConstructor2(Gen.int(), cff),
